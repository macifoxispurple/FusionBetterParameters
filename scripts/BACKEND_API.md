...
| `modelParameterCount` | `number` | Total model parameter count across all components. `0` when no document is open. Used to show section headers and load indicators without fetching the full list. Actual parameter data is fetched on demand via `getModelParameters`. |
...
| `componentId` | `string` | Stable identity token for the owning component. Normal components use `Component.entityToken`; the root component uses the stable sentinel `"root"` because Fusion rewrites the root component display name on save/version changes. Empty string only when unavailable for non-root components. Use this (not `componentName`) as the key for persistent group-order state. |
| `isRootComponent` | `boolean` | `true` when the owning component is `design.rootComponent`; FE should display this group as `Root Component` and persist it under the stable root component id. |
...
| Field | Type | Notes |
|---|---|---|
| `componentId` | `string` | `Component.entityToken` for normal owning components; `"root"` for the design root component. Stable across renames. Empty string when unavailable for non-root components. |
...
- Source: `Component.entityToken` (Fusion's own stable persistent token) for normal components; BetterParameters root sentinel `"root"` for `design.rootComponent`.
- Stable across: component renames, parameter refreshes, within a design session.
- Not guaranteed across: design close/reopen for normal component entity tokens (entityTokens are session-scoped in Fusion). The `"root"` sentinel is stable by design. FE should treat a missing match on reload as a new component and append it — preserve+append drift handling, not hard failure.
- Empty string: returned for non-root components when token is inaccessible (exception from Fusion API).
