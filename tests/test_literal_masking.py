...
class TestMaskLiterals:
    ...
    def test_single_quoted_basic(self):
        masked, err = _mask("'hello world'")
        assert err is None
        assert masked == "0"
        assert not re.search('[A-Za-z_]', masked)

    def test_single_quoted_escaped_inner_quote(self):
        masked, err = _mask("'it''s good'")
        assert err is None
        assert masked == "0"
...
class TestValidateExpressionLiterals:
    ...
    def test_single_quoted_literal_ok(self):
        with _patched_validate() as validate:
            result = validate("'text text'")
        assert result["ok"] is True, result.get("message")
    ...
    def test_text_unit_plain_text_ok(self):
        with _patched_validate() as validate:
            result = validate("hello world", units="text")
        assert result["ok"] is True, result.get("message")

    def test_text_unit_backticks_ok(self):
        with _patched_validate() as validate:
            result = validate("`hello world`", units="text")
        assert result["ok"] is True, result.get("message")
    ...
    def test_text_unit_single_quoted_ok(self):
        with _patched_validate() as validate:
            result = validate("'hello world'", units="text")
        assert result["ok"] is True, result.get("message")
...
def test_preview_expression_text_unit_returns_unwrapped_preview():
    with _patched_validate():
        result = bp._preview_expression_response("`hello world`", "", "text", "")
    assert result["ok"] is True
    assert result["preview"] == "hello world"


def test_text_expression_translation_roundtrip():
    fusion_text = bp._normalize_text_expression_for_fusion("`O'Hare`")
