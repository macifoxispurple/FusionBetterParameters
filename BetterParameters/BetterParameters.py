...
_READ_ONLY_ACTIONS = [
    ...
    "rapidCreateValidateBatch", "rapidCreatePreviewBatch",
...
def _handle_palette_action(action, data):
    ...
    simple_mutating_handlers = {
        ...
        "saveTextTunerState": lambda payload: _save_text_tuner_state(payload.get("values") if isinstance(payload, dict) else {}),
        "checkForUpdates": lambda _data: _save_update_check(_latest_release_info(force_refresh=True)),
        "downloadAndStageUpdate": lambda _data: _stage_update_payload(_latest_release_info(force_refresh=True, allow_cached_on_error=False)),
        "reinstallCurrentVersion": lambda _data: _stage_update_payload(_release_info_for_version(_current_addin_version())),
    }

    def _handle_import_parameters(payload):
        dry_run = bool(payload.get("dryRun", False))
        result = _import_parameters(payload, dry_run=dry_run)
    ...
    def _handle_preview_import_data_panel(payload):
        result = _preview_import_parameters_from_data_panel(payload)
    ...
    def _handle_import_from_data_panel(payload):
        dry_run = bool(payload.get("dryRun", False))
        result = _import_parameters_from_data_panel(payload, dry_run=dry_run)
        if result.get("cancelled"):
            return _cancelled_response(
                "Import cancelled.",
                selectionToken="",
    ...
    def _handle_retry_import_from_data_panel(payload):
        dry_run = bool(payload.get("dryRun", False))
        result = _retry_import_parameters_from_data_panel(payload, dry_run=dry_run)
        state = _current_state_payload() if (result["ok"] and not dry_run) else None
        return {
            "ok": result["ok"],
            "message": result["message"],
    ...
    complex_handlers = {
        ...
        "rapidCreateApplyBatch": lambda payload: (lambda result: {**result, "state": _current_state_payload()})(_rapid_create_apply_batch(payload)),
        "importParameters": _handle_import_parameters,
        ...
        })(_sync_metadata_json_to_fusion()),
        "syncMetadataFusionToJson": lambda _payload: (lambda sync_result: {
            **_ok_state(_current_state_payload()),
            "syncResult": sync_result,
            "debugMetadata": _collect_metadata_debug_snapshot(),
        })(_sync_metadata_fusion_to_json()),
        "repairMetadata": lambda _payload: (lambda sync_result: {
            **_ok_state(_current_state_payload()),
            "syncResult": sync_result,
            "debugMetadata": _collect_metadata_debug_snapshot(),
        })(_repair_metadata()),
        "seedTestParameters": lambda payload: (lambda result: {
            "ok": result["ok"],
            "message": result["message"],
            "state": _current_state_payload() if result["ok"] else None,
            "seededCount": result["seededCount"],
            "failedRows": result["failedRows"],
        })(_seed_test_parameters(payload)),
        "resetTestState": lambda payload: (lambda result: {**_ok_state(_current_state_payload()), "clearedCount": result["clearedCount"]})(_reset_test_state(payload)),
    }
    ...
    readonly_handlers = {
        ...
        "previewExpression": lambda payload: {
            **_preview_expression_response(
                payload.get("expression", ""),
                payload.get("currentParameterName", ""),
                payload.get("units", ""),
                payload.get("fallbackPreview", "")
            ),
            "state": None,
        },
        "validateUnit": lambda payload: {**_validate_unit_response(payload.get("unit", "")), "state": None},
        "openHelpUrl": lambda payload: {**_open_help_url(payload), "state": None},
        "copyToClipboard": lambda payload: {**_copy_to_clipboard(payload), "state": None},
        "getActiveDocumentInfo": lambda _payload: _ok_data(document=_active_document_info()),
        "getMetadataDebugSnapshot": lambda _payload: _ok_data(debugMetadata=_collect_metadata_debug_snapshot()),
        "getModelParameters": lambda payload: {**_get_model_parameters(payload), "state": None},
        "getParameterDependencyGraph": lambda _payload: (lambda graph: _ok_data(nodes=graph["nodes"], edges=graph["edges"]))(_get_parameter_dependency_graph()),
        "getBackendContractInfo": lambda _payload: _ok_data(**_get_backend_contract_info()),
        "runSelfTestSuite": lambda payload: (lambda result: _ok_data(
            totalCount=result["totalCount"],
            passedCount=result["passedCount"],
            failedCount=result["failedCount"],
            results=result["results"],
        ))(_run_self_test_suite(payload)),
        "rapidCreateValidateBatch": lambda payload: {**_rapid_create_validate_batch(payload), "state": None},
        "rapidCreatePreviewBatch": lambda payload: {**_rapid_create_preview_batch(payload), "state": None},
    ...
    if action in simple_mutating_handlers:
        row_key = str(request_data.get("key") or request_data.get("name") or "")
        simple_mutating_handlers[action](request_data)
        return _stateful_ok_response(data=request_data, action=action, row_key=row_key)

    if action in complex_handlers:
        return complex_handlers[action](request_data)

    if action in readonly_handlers:
        return readonly_handlers[action](request_data)
...
def _rapid_create_normalize_rows(data):
    rows = data.get("rows")
    if not isinstance(rows, list):
        raise BPValidationError('"rows" must be an array.')
    normalized = []
    for index, raw in enumerate(rows):
        item = raw if isinstance(raw, dict) else {}
        normalized.append({
            "index": index,
            "rowId": str(item.get("rowId") or f"rapid-row-{index + 1}"),
            "matchedParameterKey": str(item.get("matchedParameterKey") or "").strip(),
            "currentName": str(item.get("currentName") or "").strip(),
            "targetName": str(item.get("targetName") or "").strip(),
            "expression": str(item.get("expression") or "").strip(),
            "unit": str(item.get("unit") or "").strip(),
            "comment": str(item.get("comment") or ""),
            "operation": str(item.get("operation") or "").strip().lower() or "create",
        })
    return normalized


def _rapid_create_find_parameter(design, row):
    key = str(row.get("matchedParameterKey") or "").strip()
    current_name = str(row.get("currentName") or "").strip()
    parameter = _find_user_parameter_by_token(design, key) if key else None
    if not parameter and current_name:
        parameter = design.userParameters.itemByName(current_name)
    return parameter


def _rapid_create_extract_dependencies(expression, candidate_names):
    if not expression:
        return []
    masked_expr, literal_error = _mask_backtick_literals_only(expression)
    if literal_error or masked_expr is None:
        return []
    candidate_map = {str(name or "").strip().lower(): str(name or "").strip() for name in candidate_names if str(name or "").strip()}
    dependencies = []
    seen = set()
    for match in EXPRESSION_TOKEN_PATTERN.finditer(masked_expr):
        token = str(match.group(0) or "").strip()
        if not token:
            continue
        lowered = token.lower()
        if lowered not in candidate_map:
            continue
        if lowered in seen:
            continue
        dependencies.append(candidate_map[lowered])
        seen.add(lowered)
    return dependencies


def _rapid_create_dependency_order(prepared_rows):
    name_to_row = {}
    indegree = {}
    edges = {}
    for row in prepared_rows:
        if not row.get("targetName") or row.get("operation") == "skip":
            continue
        key = str(row["targetName"]).strip().lower()
        name_to_row[key] = row
        indegree[key] = 0
        edges[key] = set()
    ...
    for row in prepared_rows:
        ...
        if not target_name or row.get("operation") == "skip":
            continue
        row_key = target_name.lower()
        for dep in row.get("dependencies") or []:
            dep_key = str(dep or "").strip().lower()
            if not dep_key or dep_key == row_key:
                continue
            if dep_key not in name_to_row:
                continue
            if row_key in edges[dep_key]:
                continue
            edges[dep_key].add(row_key)
            indegree[row_key] += 1
    ...
    while queue:
        ...
        for neighbor in sorted(edges.get(current) or []):
            indegree[neighbor] -= 1
            if indegree[neighbor] == 0:
                queue.append(neighbor)

    cyclic = [name_to_row[key] for key, degree in indegree.items() if degree > 0]
    return {
        "order": [name_to_row[key] for key in ordered_keys if key in name_to_row],
        "cyclic": cyclic,
    }


def _rapid_create_prepare_rows(data):
    design = _require_design()
    rows = _rapid_create_normalize_rows(data if isinstance(data, dict) else {})
    candidate_names = [row["targetName"] for row in rows if row.get("targetName")]
    prepared = []
    duplicate_targets = {}
    for row in rows:
        target_key = str(row.get("targetName") or "").strip().lower()
        if target_key:
            duplicate_targets[target_key] = duplicate_targets.get(target_key, 0) + 1

    for row in rows:
        operation = row["operation"] if row["operation"] in {"create", "update", "rename", "skip"} else "create"
        matched_param = _rapid_create_find_parameter(design, row)
        matched_name = str(matched_param.name or "") if matched_param else ""
        matched_key = _parameter_entity_token(matched_param) if matched_param else ""
        diagnostics = []

        if operation != "skip":
            if not row["targetName"]:
                diagnostics.append({"level": "error", "message": "Target name is required."})
            else:
                name_validation = _validate_parameter_name_response(
                    row["targetName"],
                    matched_name if operation in {"update", "rename"} else "",
                    matched_key if operation in {"update", "rename"} else "",
                )
                if not name_validation["ok"]:
                    diagnostics.append({"level": "error", "message": name_validation["message"]})

            if not row["expression"]:
                diagnostics.append({"level": "error", "message": "Expression is required."})

        if operation in {"update", "rename"} and not matched_param:
            diagnostics.append({"level": "error", "message": "Existing parameter match is required for update/rename."})

        if operation == "create" and matched_param and not row["targetName"]:
            diagnostics.append({"level": "error", "message": "Create rows need a target name."})

        if row["targetName"] and duplicate_targets.get(str(row["targetName"]).lower(), 0) > 1 and operation != "skip":
            diagnostics.append({"level": "error", "message": f'Duplicate target name "{row["targetName"]}" in batch.'})

        normalized_unit = row["unit"]
        if normalized_unit:
            unit_validation = _validate_unit_response(normalized_unit)
            if not unit_validation.get("ok"):
                diagnostics.append({"level": "error", "message": unit_validation.get("message") or "Invalid unit."})
            else:
                normalized_unit = str(unit_validation.get("unit") or normalized_unit)

        normalized_expression = _normalize_expression_for_fusion(row["expression"], normalized_unit)
        dependencies = _rapid_create_extract_dependencies(normalized_expression, candidate_names)
        if row["targetName"]:
            dependencies = [dep for dep in dependencies if dep.strip().lower() != row["targetName"].strip().lower()]

        validation = None
        if operation != "skip" and row["expression"]:
            validation = _validate_expression_response(
                normalized_expression,
                row["targetName"],
                normalized_unit,
            )
            if not validation.get("ok"):
                diagnostics.append({"level": "error", "message": validation.get("message") or "Invalid expression."})

        prepared.append({
            **row,
            "operation": operation,
            "matchedParameter": matched_param,
            "matchedCurrentName": matched_name,
            "matchedParameterKey": matched_key,
    ...
    dependency_result = _rapid_create_dependency_order(prepared)
    cyclic_rows = {row["rowId"] for row in dependency_result["cyclic"]}
    if cyclic_rows:
        for row in prepared:
            if row["rowId"] in cyclic_rows:
                row["diagnostics"].append({"level": "error", "message": "Dependency cycle detected inside this batch."})

    return {
        "rows": prepared,
        "dependencyOrder": [str(row.get("targetName") or "") for row in dependency_result["order"] if str(row.get("targetName") or "").strip()],
    }


def _rapid_create_validate_batch(data):
    prepared = _rapid_create_prepare_rows(data if isinstance(data, dict) else {})
    row_results = []
    blocking_count = 0
    warning_count = 0
    for row in prepared["rows"]:
        diagnostics = list(row.get("diagnostics") or [])
        has_error = any(str(item.get("level") or "") == "error" for item in diagnostics)
        warning_count += sum(1 for item in diagnostics if str(item.get("level") or "") == "warning")
        if has_error:
            blocking_count += 1
        row_results.append({
            "rowId": row["rowId"],
            "operation": row["operation"],
            "matchedCurrentName": row.get("matchedCurrentName", ""),
            "matchedParameterKey": row.get("matchedParameterKey", ""),
            "targetName": row.get("targetName", ""),
            "normalizedUnit": row.get("normalizedUnit", ""),
    ...
    return {
        "ok": blocking_count == 0,
        "message": "" if blocking_count == 0 else f"{blocking_count} rapid-create row(s) need attention.",
        "rows": row_results,
        "dependencyOrder": list(prepared.get("dependencyOrder") or []),
        "blockingCount": blocking_count,
        "warningCount": warning_count,
    }


def _rapid_create_preview_batch(data):
    prepared = _rapid_create_prepare_rows(data if isinstance(data, dict) else {})
    row_results = []
    blocking_count = 0
    for row in prepared["rows"]:
        ...
        if has_error:
            blocking_count += 1
        elif row["operation"] != "skip":
            preview_result = _preview_expression_response(
            ...
            if not preview_result.get("ok"):
                diagnostics.append({"level": "error", "message": preview_result.get("message") or "Preview failed."})
                has_error = True
                blocking_count += 1
            else:
                preview = str(preview_result.get("preview") or "")
        row_results.append({
            "rowId": row["rowId"],
            "operation": row["operation"],
            "preview": preview,
    ...
    return {
        "ok": blocking_count == 0,
        "message": "" if blocking_count == 0 else f"{blocking_count} rapid-create preview(s) failed.",
        "rows": row_results,
        "dependencyOrder": list(prepared.get("dependencyOrder") or []),
        "blockingCount": blocking_count,
    }
...
def _rapid_create_apply_batch(data):
    prepared = _rapid_create_prepare_rows(data if isinstance(data, dict) else {})
    row_results = []
    blocking_rows = []
    for row in prepared["rows"]:
    ...
    if blocking_rows:
        return {
            "ok": False,
            "message": f"{len(blocking_rows)} rapid-create row(s) failed validation.",
            "rows": row_results,
            "counts": {"create": 0, "update": 0, "rename": 0, "skip": 0, "error": len(blocking_rows)},
            "dependencyOrder": list(prepared.get("dependencyOrder") or []),
        }

    design = _require_design()
    counts = {"create": 0, "update": 0, "rename": 0, "skip": 0, "error": 0}
    rows_by_id = {result["rowId"]: result for result in row_results}

    create_rows = [row for row in prepared["rows"] if row["operation"] == "create"]
    rename_rows = [row for row in prepared["rows"] if row["operation"] == "rename"]
    update_rows = [row for row in prepared["rows"] if row["operation"] == "update"]
    skip_rows = [row for row in prepared["rows"] if row["operation"] == "skip"]

    for row in skip_rows:
        counts["skip"] += 1
        rows_by_id[row["rowId"]]["message"] = "Skipped."

    for row in rename_rows:
        result = rows_by_id[row["rowId"]]
        try:
            _rename_parameter({
                "key": row.get("matchedParameterKey", ""),
                "name": row.get("matchedCurrentName", ""),
                "newName": row.get("targetName", ""),
            })
            counts["rename"] += 1
            ...
            result["diagnostics"] = [{"level": "error", "message": str(exc)}]
            counts["error"] += 1

    ordered_creates = list(_rapid_create_dependency_order(create_rows)["order"])
    seen_create_ids = {row["rowId"] for row in ordered_creates}
    ordered_creates.extend([row for row in create_rows if row["rowId"] not in seen_create_ids])
    for row in ordered_creates:
        result = rows_by_id[row["rowId"]]
        if not result["ok"]:
            continue
        try:
            _create_parameter({
                ...
                "expression": row.get("normalizedExpression", ""),
                "unit": row.get("normalizedUnit", ""),
    ...
    batch_updates = []
    update_rows_for_comments = []
    for row in rename_rows + update_rows:
        result = rows_by_id[row["rowId"]]
        if not result["ok"]:
            continue
        parameter = _rapid_create_find_parameter(design, {
            "matchedParameterKey": row.get("matchedParameterKey", ""),
            "currentName": row.get("targetName", ""),
        })
        if not parameter:
            result["ok"] = False
            result["message"] = "User parameter was not found after rename."
            result["diagnostics"] = [{"level": "error", "message": result["message"]}]
            counts["error"] += 1
            continue
        batch_updates.append({
            "key": _parameter_entity_token(parameter),
            "name": str(parameter.name or row.get("targetName", "")),
            "expression": row.get("normalizedExpression", ""),
        })
        update_rows_for_comments.append((row, parameter))

    if batch_updates:
        update_result = _batch_update_parameters({"updates": batch_updates})
        if not update_result.get("ok"):
            failed_by_name = {
                str(item.get("name") or ""): str(item.get("message") or "Update failed.")
                for item in update_result.get("failedRows") or []
            }
            for row, parameter in update_rows_for_comments:
                result = rows_by_id[row["rowId"]]
                target_name = str(parameter.name or row.get("targetName", ""))
                if target_name in failed_by_name:
                    result["ok"] = False
                    result["message"] = failed_by_name[target_name]
                    result["diagnostics"] = [{"level": "error", "message": result["message"]}]
                    counts["error"] += 1
                else:
        ...
        for row, parameter in update_rows_for_comments:
            ...
            if not result["ok"]:
                continue
            try:
                parameter.comment = str(row.get("comment") or "")
            except Exception:
                pass
    ...
    return {
        "ok": ok,
        "message": "" if ok else f"{counts['error']} rapid-create row(s) failed during apply.",
        "rows": row_results,
        "counts": counts,
        "dependencyOrder": list(prepared.get("dependencyOrder") or []),
    }
...
Notes:
  ...
  - `dev/` and `tests/` are always skipped so mock/dev and test files are never
    pushed into the live Fusion add-in directory.
"""
import hashlib
import os
import shutil
...
ALWAYS_SKIP = {".git", ".gitignore", "dev", "tests"}
